...
if run:
 ...
 with st.status("Researching...",expanded=True) as status:
  discovery=DiscoveryAgent(g).run(niche,subs_list,manual); st.write(f"🔎 Community: {len(discovery['posts'])} evidence items, {len(discovery['quotes'])} quotes")
  if discovery.get("error"):
   st.warning(f"Community source note: {discovery['error']}")
